...
class GenotypeDataset(Dataset):
    ...
    def __init__(self, vec_basename, sample_name, vector_names):
        ...
        self.reader=VectorReader(vec_basename, sample_name=sample_name, vector_names=vector_names, return_example_id=True)
    ...
    def __getitem__(self, idx):
        # get next example from .vec file and check that idx matches example index, then return the features and outputs as tuple.
        example_index, tuple= next(self.reader)
        assert example_index==idx, "Requested example index out of order of .vec file."
        return tuple
