...
class SemiSupAdvEncoder(nn.Module):
    ...
    def forward(self, input):
        if input.data.is_cuda and self.ngpus > 1:
            cat_encoded = nn.parallel.data_parallel(self.cat_encoder, input, self.device_list)
            prior_encoded = nn.parallel.data_parallel(self.prior_encoder, input, self.device_list)
        else:
            cat_encoded = self.cat_encoder(input)
            prior_encoded = self.prior_encoder(input)
...
class SemiSupAdvDecoder(nn.Module):
    ...
    def forward(self, input):
        if input.data.is_cuda and self.ngpus > 1:
            decoded = nn.parallel.data_parallel(self.decoder, input, self.device_list)
        else:
...
class SemiSupAdvDiscriminatorCat(nn.Module):
    ...
    def forward(self, input):
        if input.data.is_cuda and self.ngpus > 1:
            category = nn.parallel.data_parallel(self.cat_discriminator, input, self.device_list)
        else:
            category = self.cat_discriminator(input)
        return category
...
class SemiSupAdvDiscriminatorPrior(nn.Module):
    ...
    def forward(self, input):
        if input.data.is_cuda and self.ngpus > 1:
            prior_val = nn.parallel.data_parallel(self.prior_discriminator, input, self.device_list)
        else:
            prior_val = self.prior_discriminator(input)
        return prior_val

def create_semisup_adv_autoencoder_model(model_name, problem, encoded_size=32, ngpus=1, nreplicas=1, dropout_p=0, N_dim=500,
        num_hidden_layers=1, z_dim=2):
