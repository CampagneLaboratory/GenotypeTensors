...
class AdversarialAutoencoderTrainer(CommonTrainer):
    ...
    def train_semisup_aae(self, epoch,
        ...
        if self.args.latent_code_output is not None:
            ...
            latent_code_tensor = torch.stack(self.latent_codes).cpu()
            ...
            gaussian_code_tensor = torch.stack(self.gaussian_codes).cpu()
        ...
        try:
            for batch_idx, (_, data_dict) in enumerate(data_provider):
                ...
                self.test_one_batch(performance_estimators,
                                   batch_idx, input_s, target_s, meta_data, errors)
