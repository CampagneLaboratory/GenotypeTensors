...
class Trainer_AE:
    ...
    def training_supervised(self):
        ...
        for epoch in range(self.start_epoch, self.start_epoch + self.args.num_epochs):
            ...
            perfs = PerformanceList()
            if self.args.mode == "autoencoder":
                perfs += self.autoencoder(epoch)
            elif self.args.mode == "supervised_somatic":
                perfs += self.supervised_somatic(epoch)

            perfs += [lr_train_helper]
            if previous_test_perfs is None or self.epoch_is_test_epoch(epoch):
                if self.args.mode == "autoencoder":
                    perfs += self.test_autoencoder(epoch)
                elif self.args.mode == "supervised_somatic":
                    perfs += self.test_somatic_classifer(epoch)
